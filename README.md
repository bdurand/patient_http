...
config = PatientHttp::Configuration.new(
  ...
  # Retries for failed requests (default: 3). Independently of this setting, a
  # request that fails before any response byte arrives is retried at once on a
  # new connection when that is known to be safe: the server refused it before
  # processing it (for example on an HTTP/2 GOAWAY), the write failed with
  # EPIPE so the server never received the whole request, or the method is
  # idempotent (GET, HEAD, PUT, DELETE, QUERY) and the connection failed with
  # EOF, a reset, or the kernel giving up on unacknowledged data. A POST or
  # PATCH whose connection failed with an unknown outcome is not retried, and
  # neither is a request that reached request_timeout.
  retries: 3,
