...
RSpec.describe PatientHttp::ClientPool do
  ...
  describe "protocol enforcement" do
    ...
    it "leaves endpoints unchanged when no protocol is set" do
      ...
      expect(client.endpoint).to be(original_endpoint)
    end
  end

  describe "per-host connection limits" do
    it "passes the connection limit to the HTTP client" do
      limited_pool = described_class.new(max_size: 3, connection_limit: 8)

      expect(Async::HTTP::Client).to receive(:new)
        .with(anything, hash_including(limit: 8))
        .and_call_original

      limited_pool.client_for(Async::HTTP::Endpoint.parse("https://example.com"))
      expect(limited_pool.connection_limit).to eq(8)
      limited_pool.close
    end

    it "defaults to unlimited connections per host" do
      expect(Async::HTTP::Client).to receive(:new) do |_endpoint, **options|
        expect(options).not_to have_key(:limit)
      end

      pool.client_for(Async::HTTP::Endpoint.parse("https://example.com"))
      expect(pool.connection_limit).to be_nil
    end
  end
